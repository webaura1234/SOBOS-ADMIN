      ...
      {tab === "audit" && (
        <>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <input type="date" className="h-10 px-3 border border-border bg-surface-2 text-text-primary rounded-xl font-bold text-sm" value={from} onChange={(e) => setFrom(e.target.value)} aria-label="From date" />
            <span className="text-text-muted">–</span>
            <input type="date" className="h-10 px-3 border border-border bg-surface-2 text-text-primary rounded-xl font-bold text-sm" value={to} onChange={(e) => setTo(e.target.value)} aria-label="To date" />
          </div>
          <DenseGrid columns={logCols} data={logs} selectable={false} onRowClick={setSelectedLog} emptyMessage="No audit entries match" />
          {selectedLog && (
            <div className="mt-4 p-4 bg-surface-1 border border-border rounded-xl text-text-primary shadow-2xs">
              <h3 className="font-bold mb-3 text-text-primary">Change Detail — {selectedLog.action} {selectedLog.resourceType} {selectedLog.resourceId ? `(${selectedLog.resourceId})` : ""}</h3>
              <div className="grid grid-cols-2 gap-4 text-sm font-mono">
                <div><p className="font-bold mb-1 text-text-primary">Before</p><pre className="bg-surface-2 border border-border text-yellow p-3 rounded-lg overflow-auto max-h-72">{selectedLog.beforeJson ? JSON.stringify(JSON.parse(selectedLog.beforeJson), null, 2) : "—"}</pre></div>
                <div><p className="font-bold mb-1 text-text-primary">After</p><pre className="bg-surface-2 border border-border text-yellow p-3 rounded-lg overflow-auto max-h-72">{selectedLog.afterJson ? JSON.stringify(JSON.parse(selectedLog.afterJson), null, 2) : "—"}</pre></div>
              </div>
              <p className="text-xs text-text-muted mt-2">Audit log is immutable — entries can&apos;t be edited or deleted. Export is platform-admin only.</p>
            </div>
          )}
        </>
      )}
