  ...
  return (
    <div className={cn("space-y-6 transition-opacity", loading && "opacity-60")}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 rounded-2xl bg-surface-1 border border-border shadow-2xs flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-yellow/20 text-yellow flex items-center justify-center font-bold shrink-0">
        ...
        <div className="p-4 rounded-2xl bg-surface-1 border border-border shadow-2xs flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-red-surface text-red border border-[var(--border-critical)] flex items-center justify-center font-bold shrink-0">
            <Flame size={20} />
          </div>
          <div>
            <div className="text-xs font-bold text-text-muted uppercase tracking-wider">
        ...
        <div className="p-4 rounded-2xl bg-surface-1 border border-border shadow-2xs flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xl font-extrabold text-text-primary mt-0.5 tabular-nums">
      ...
      <div className="p-5 rounded-2xl bg-surface-1 border border-border shadow-2xs space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-border pb-3">
          ...
          {hoverCell ? (
            <div className="flex items-center gap-2.5 px-3.5 py-1.5 rounded-xl bg-surface-2 text-text-primary border border-border text-xs font-bold shadow-md animate-in fade-in duration-150 self-start md:self-auto">
              ...
              <span className="text-text-muted">·</span>
              <span className="text-yellow">{formatCurrency(hoverCell.revenue)}</span>
              {hoverCell.count > 0 && (
                <>
                  <span className="text-text-muted">·</span>
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-text-muted self-start md:self-auto">
              <span>Low</span>
              <div className="flex items-center gap-1">
                <span className="w-5 h-5 rounded-md bg-surface-2 border border-border flex items-center justify-center text-[10px] text-text-muted/40 font-semibold" title="0 orders">·</span>
                <span className="w-5 h-5 rounded-md bg-yellow/20 border border-yellow/40 flex items-center justify-center text-[10px] text-yellow font-bold" title="1-2 orders">1</span>
                <span className="w-5 h-5 rounded-md bg-yellow/45 border border-yellow/60 flex items-center justify-center text-[10px] text-yellow font-bold" title="3-4 orders">3</span>
                <span className="w-5 h-5 rounded-md bg-yellow/75 border border-yellow-hover flex items-center justify-center text-[10px] text-[var(--on-yellow)] font-extrabold" title="5-7 orders">6</span>
                <span className="w-5 h-5 rounded-md bg-yellow border border-yellow-hover shadow-2xs flex items-center justify-center text-[10px] text-[var(--on-yellow)] font-black" title="8-10+ orders">10</span>
              </div>
              <span className="text-text-primary font-extrabold">High</span>
            </div>
          )}
        </div>
            ...
            <table className="w-full border-separate border-spacing-1.5 min-w-[700px]">
                ...
                <tr>
                  ...
                  {PEAK_HOURS.map((h) => {
                    ...
                    return (
                      <th
                        key={h}
                        className="text-[11px] font-extrabold text-text-muted uppercase tracking-wider text-center pb-2 select-none"
                      >
                        {formatted}
                      </th>
                    );
              ...
              <tbody>
                {DAYS.map((dayLabel, dayIdx) => {
                  ...
                  return (
                    <tr key={dayLabel}>
                      <td className="text-xs font-extrabold text-text-primary pr-2 text-right select-none">
                        {dayLabel}
                      </td>
                        ...
                        if (hasOrders) {
                          if (intensity > 0.6) {
                            bgStyle = {
                              ...
                              borderColor: "#FED500",
                              boxShadow: "0 2px 5px rgba(254, 213, 0, 0.35)",
                            };
                          } else {
                            const alpha = 0.25 + intensity * 0.5;
                            bgStyle = {
                              backgroundColor: `rgba(254, 213, 0, ${alpha.toFixed(2)})`,
                              borderColor: "rgba(254, 213, 0, 0.5)",
                            };
                          }
                        }
                              ...
                              {hasOrders ? (
                                <span
                                  className={cn(
                                    ...
                                    intensity > 0.6 ? "text-[#0A0A0A]" : "text-yellow"
                                  )}
                                >
                                  {cell.count}
                                </span>
